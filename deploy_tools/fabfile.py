...
REPO_URL = 'https://github.com/Moecker/tdd_python.git'
...
def _update_pip(source_folder):
    run('pip3 install -r requirements.txt')


def _update_static_files(source_folder):
    run(
        'cd {} && ../virtualenv/bin/python manage.py collectstatic --noinput'.format(source_folder)
    )


def _update_database(source_folder):
    run(
        'cd {} && ../virtualenv/bin/python manage.py migrate --noinput'.format(source_folder)
    )
